...
poly::poly(uint8_t numDimension_input, uint16_t numTerms_input, double* data_mat)
{
	...
	degree = new uint8_t[numTerms * numDimension];
	maxDegree = new uint8_t[numDimension];
	for (uint16_t i = 0; i < numDimension; i++)
	{
		maxDegree[i] = 0;
		for (uint16_t j = 0; j < numTerms; j++)
		{
			degree[j * numDimension + i] = (uint8_t)data_mat[j * (numDimension + 1) + i];
...
poly::poly(uint8_t numDimension_input, uint16_t numTerms_input, float* data_mat)
{
	numDimension = numDimension_input;
	numTerms = numTerms_input;
...
poly::poly(uint8_t numDimension_input, uint16_t numTerms_input, double* degree_input, double* coef_input) {
	numDimension = numDimension_input;
	numTerms = numTerms_input;
	...
	for (uint16_t i = 0; i < numDimension; i++)
	{
		...
		for (uint16_t j = 0; j < numTerms; j++)
		{
			degree[j * numDimension + i] = (uint8_t)degree_input[j * numDimension + i];
			if (degree[j * numDimension + i] > maxDegree[i])
			{
				maxDegree[i] = degree[j * numDimension + i];
			}
...
void poly::printDetails() {
	...
	for (uint16_t i = 0; i < numTerms; i++) {
		...
		for (uint16_t j = 0; j < numDimension; j++) {
...
void poly::partialDerivative(poly* &res, float &res_value, uint8_t dim) {
	float* data_mat = new float[numTerms * (numDimension + 1)];
	uint16_t pd_numTerms = 0;
	float pd_value = 0;
	for (uint16_t i = 0; i < numTerms; i++) {
